        ...
        "dist/doc.js"(exports2, module3) {
          ...
          var require_string_width = __commonJS22({
            "vendors/string-width.js"(exports22, module22) {
              ...
              module22.exports = __toCommonJS2(string_width_exports);
              function ansiRegex() {
                let {
                  onlyFirst = false
...
// index.ts
var unified_latex_prettier_exports = {};
__export(unified_latex_prettier_exports, {
  prettierPluginLatex: () => prettierPluginLatex,
  printLatexAst: () => printLatexAst,
  printer: () => printLatexAst
});
module.exports = __toCommonJS(unified_latex_prettier_exports);
...
function formatEnvSurround(node) {
  const env = (0, import_unified_latex_util_print_raw.printRaw)(node.env);
  return {
    envName: env,
...
function formatDocArray(nodes, docArray, options) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    switch (rawNode.type) {
      case "comment":
        if (!rawNode.sameline && previousNode && !import_unified_latex_util_match.match.comment(previousNode) && !import_unified_latex_util_match.match.parbreak(previousNode)) {
          ret.push(hardline);
        }
      ...
      case "macro":
        if (renderInfo.breakBefore || renderInfo.breakAround) {
          if (previousNode) {
            if (ret[ret.length - 1] === line || ret[ret.length - 1] === hardline) {
              ret.pop();
              ret.push(hardline);
            } else if (!import_unified_latex_util_match.match.comment(previousNode) && !import_unified_latex_util_match.match.parbreak(previousNode)) {
              ret.push(hardline);
            }
          }
        }
        ...
        if (renderInfo.breakAfter || renderInfo.breakAround) {
          if (nextNode) {
            if (import_unified_latex_util_match.match.whitespace(nextNode)) {
              ret.push(hardline);
              i++;
            } else if (import_unified_latex_util_match.match.parbreak(nextNode)) {
            } else if (!import_unified_latex_util_match.match.comment(nextNode)) {
              ret.push(hardline);
            }
          }
        }
...
var import_unified_latex_util_pgfkeys = require("@unified-latex/unified-latex-util-pgfkeys");
...
function printArgument(path, print, options) {
  ...
  if (import_unified_latex_util_match2.match.comment(node.content[node.content.length - 1])) {
  ...
  if (path.getParentNode()) {
    ...
    if (parentRenderInfo.pgfkeysArgs) {
      const leadingComment = node.content.length > 0 && import_unified_latex_util_match2.match.comment(node.content[0]) && node.content[0].sameline ? node.content[0] : null;
      const content2 = leadingComment ? node.content.slice(1) : node.content;
      (0, import_unified_latex_util_trim.trim)(content2);
...
function printPgfkeysArgument(nodes, braces) {
  const parsed = (0, import_unified_latex_util_pgfkeys.parsePgfkeys)(nodes);
  const content = [];
  for (const part of parsed) {
    const isLastItem = part === parsed[parsed.length - 1];
    if (part.itemParts) {
      const parts = part.itemParts.map((node) => (0, import_unified_latex_util_print_raw2.printRaw)(node, { asArray: true }).map((token) => token === import_unified_latex_util_print_raw2.linebreak ? hardline : token));
...
function hasPreambleCode(nodes) {
  return nodes.some((node) => import_unified_latex_util_match3.match.macro(node, "documentclass"));
}
...
function printComment(path, _print, _options) {
  ...
  const content = [leadingWhitespace, "%" + (0, import_unified_latex_util_print_raw3.printRaw)(node.content)];
  return content;
}
...
var import_unified_latex_util_print_raw4 = require("@unified-latex/unified-latex-util-print-raw");
var import_unified_latex_util_align = require("@unified-latex/unified-latex-util-align");
...
function printAlignedEnvironment(path, print, options) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    content.push(row);
    if (rowSep) {
      content.push((0, import_unified_latex_util_print_raw4.printRaw)(rowSep));
    }
    if (rowSep && trailingComment) {
      content.push(" ");
    }
    if (trailingComment) {
      content.push(["%", (0, import_unified_latex_util_print_raw4.printRaw)(trailingComment.content)]);
    }
    if (rowSep || trailingComment) {
      content.push(hardline);
    }
  ...
  if (leadingComment) {
    content.unshift(leadingComment.leadingWhitespace ? " " : "", "%" + (0, import_unified_latex_util_print_raw4.printRaw)(leadingComment.content), hardline);
...
function formatAlignedContent(nodes) {
  ...
  const rows = (0, import_unified_latex_util_align.parseAlignEnvironment)(nodes);
  const numCols = Math.max(...rows.map((r) => r.cells.length));
  const rowSeps = rows.map(({ rowSep }) => (0, import_unified_latex_util_print_raw4.printRaw)(rowSep || []));
  ...
  const renderedRows = rows.map(({ cells, colSeps }) => ({
    cells: cells.map((nodes2) => {
      (0, import_unified_latex_util_trim2.trim)(nodes2);
      return (0, import_unified_latex_util_print_raw4.printRaw)(nodes2);
    }),
    seps: colSeps.map((nodes2) => (0, import_unified_latex_util_print_raw4.printRaw)(nodes2))
  }));
  const colWidths = [];
...
var import_unified_latex_util_print_raw5 = require("@unified-latex/unified-latex-util-print-raw");
...
var ReferenceMap = class {
  constructor(ast) {
    ...
    (0, import_unified_latex_util_visit.visit)(this.ast, (nodeList) => {
...
function printLatexAst(path, options, print) {
  ...
  switch (node.type) {
    ...
    case "group":
      const content = (0, import_unified_latex_util_print_raw5.printRaw)(node.content, {
        asArray: true
      }).map((token) => token === import_unified_latex_util_print_raw5.linebreak ? hardline : token);
      return ["{", ...content, "}"];
    case "inlinemath":
    ...
    case "verb":
      return [
        ...
        node.env,
        node.escape,
        (0, import_unified_latex_util_print_raw5.printRaw)(node.content),
        node.escape
      ];
    case "verbatim":
    ...
    default:
      ...
      return (0, import_unified_latex_util_print_raw5.printRaw)(node);
  }
}
...
var parsers = {
  "latex-parser": {
    parse: import_unified_latex_util_parse.parse,
    astFormat: "latex-ast",
