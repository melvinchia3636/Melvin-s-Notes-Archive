...
var parseCache = {};
function parse(str = "") {
  parseCache[str] = parseCache[str] || import_unified_latex_util_pegjs.XColorPegParser.parse(str);
...
function computeColor(expr, predefinedColors = {}) {
  ...
  const knownColors = __spreadValues(__spreadValues({}, PREDEFINED_XCOLOR_COLORS), predefinedColors);
  ...
  if (color.type === "extended_expr") {
    ...
    const colorsToMix = color.expressions.map((expr2) => [
      expr2.weight / div,
      computeColor({
        type: "color",
        color: expr2.color,
...
function xcolorMacroToHex(node) {
  ...
  (0, import_unified_latex_util_comments.deleteComments)(node);
  const args = (0, import_unified_latex_util_arguments.getArgsContent)(node);
  const model = args[0] && (0, import_unified_latex_util_print_raw.printRaw)(args[0]);
  const colorStr = (0, import_unified_latex_util_print_raw.printRaw)(args[1] || []);
  let hex = null;
  try {
...
function colorToTextcolorMacro(content, origMacro) {
  ...
  const args = (origMacro.args ? origMacro.args : [(0, import_unified_latex_builder.arg)([], { closeMark: "", openMark: "" }), (0, import_unified_latex_builder.arg)([])]).concat((0, import_unified_latex_builder.arg)(content));
  return {
    type: "macro",
    content: "textcolor",
    args,
